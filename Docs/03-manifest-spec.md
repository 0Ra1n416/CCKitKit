...
⚠️ `hint` 写成单个字符串会被 lint 拒绝。这是本项目自己犯过的错误(见 D-08)。
...
改为 `requires.system` 声明。违反此约束会破坏可卸载性(见 D-02)。
...
遵守此约束的白拿好处是——cckit 装的 skill 仍可直接上传到 claude.ai。
...
而用户完全看不出原因——这是最难 debug 的一类问题,所以值得在安装时就拦。
